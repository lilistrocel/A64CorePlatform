"""
Marketing Module - Registration Interface
...
async def startup_hook():
    ...
    try:
        ...
        logger.info("[Marketing Module] Database connected successfully")
    except Exception as e:
        logger.error(f"[Marketing Module] Failed to connect to database: {e}")
        raise
...
async def shutdown_hook():
    ...
    """
    logger.info("[Marketing Module] Shutting down")
    await marketing_db.disconnect()
    logger.info("[Marketing Module] Database disconnected")


def register(app: FastAPI, prefix: Optional[str] = None) -> None:
    """
    Register the Marketing module with the main application.
    ...
    This function:
    1. Registers all Marketing API routes
