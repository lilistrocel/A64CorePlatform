...
class PerformanceReviewBase(BaseModel):
    ...
    happinessScore: int = Field(
        ..., ge=1, le=10, description="Employee happiness score (1-10)"
    )
    strengths: List[str] = Field(
        default_factory=list, description="List of employee strengths"
    )
    areasForImprovement: List[str] = Field(
        default_factory=list, description="Areas for improvement"
    )
    goals: List[str] = Field(
        default_factory=list, description="Goals for next review period"
    )
...
class PerformanceReview(PerformanceReviewBase):
    ...
    reviewId: UUID = Field(
        default_factory=uuid4, description="Unique review identifier"
    )
