...
class ChatQueryRequest(BaseModel):
    ...
    conversation_history: Optional[List[ConversationMessage]] = Field(
        default=[], max_length=10, description="Previous conversation messages (max 10)"
    )
    force_refresh: bool = Field(
        default=False, description="Skip cache and force fresh query"
    )
        ...
        json_schema_extra = {
            "example": {
                ...
                "conversation_history": [
                    {"role": "user", "content": "Show me all farms"},
                    {"role": "assistant", "content": "Found 15 farms..."},
                ],
                "force_refresh": False,
            }
        }
...
class VisualizationSuggestion(BaseModel):
    ...
    type: str = Field(
        ..., description="Chart type: bar_chart, pie_chart, line_chart, etc."
    )
    title: str = Field(..., description="Chart title")
    x_axis: Optional[str] = Field(
        None, description="X-axis field (for bar/line charts)"
    )
    y_axis: Optional[str] = Field(
        None, description="Y-axis field (for bar/line charts)"
    )
...
class ReportInfo(BaseModel):
    ...
    visualization_suggestions: List[VisualizationSuggestion] = Field(
        default=[], description="Suggested charts/graphs"
    )
...
class CostInfo(BaseModel):
    ...
    query_generation: Dict[str, Any] = Field(
        ..., description="Cost for query generation"
    )
    report_generation: Dict[str, Any] = Field(
        ..., description="Cost for report generation"
    )
...
class CollectionFieldInfo(BaseModel):
    ...
    appears_in_percent: float = Field(
        ..., description="Percentage of documents with field"
    )
...
class CollectionIndexInfo(BaseModel):
    ...
    keys: Dict[str, Union[int, str]] = Field(
        ...,
        description="Index keys and direction (int for normal, str for text indexes)",
    )
...
class CollectionInfo(BaseModel):
    ...
    relationships: List[CollectionRelationship] = Field(
        default=[], description="Inferred relationships"
    )
...
class SchemaResponse(BaseModel):
    ...
    collections: Dict[str, CollectionInfo] = Field(
...
class UserCostResponse(BaseModel):
    ...
    daily_breakdown: Optional[List[Dict[str, Any]]] = Field(
        None, description="Daily cost breakdown"
    )
        ...
        json_schema_extra = {
            "example": {
                "error": {
                    ...
                    "details": {"operator": "$where", "stage": 2},
                    "timestamp": "2025-11-24T12:00:00",
                }
            }
        }
