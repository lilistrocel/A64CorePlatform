...
class EnvironmentLog(EnvironmentReading):
    ...
    logId: str = Field(
        default_factory=lambda: str(uuid4()), description="Unique log ID"
    )
    roomId: str = Field(..., description="Growing room ID")
    facilityId: Optional[str] = Field(None, description="Facility ID (denormalized)")

    # Context
    currentPhase: Optional[str] = Field(
        None, description="Room phase at time of reading"
    )
    notes: Optional[str] = Field(None, max_length=500)
    recordedBy: Optional[str] = Field(None, description="User ID or sensor ID")

    # Alert flags
    isOutOfRange: bool = Field(
        False, description="Whether any reading is outside target range"
    )
