...
from src.modules.logistics.utils.responses import (
    SuccessResponse,
    PaginatedResponse,
    PaginationMeta,
)
...
async def create_vehicle(
    ...
    vehicle = await service.create_vehicle(vehicle_data, UUID(current_user.userId))

    return SuccessResponse(data=vehicle, message="Vehicle created successfully")
...
async def get_vehicles(
    ...
    status: Optional[VehicleStatus] = Query(
        None, description="Filter by vehicle status"
    ),
    type: Optional[VehicleType] = Query(None, description="Filter by vehicle type"),
    ownershipType: Optional[VehicleOwnership] = Query(
        None, description="Filter by ownership type"
    ),
    ...
    vehicles, total, total_pages = await service.get_all_vehicles(
        ...
        ownershipType.value if ownershipType else None,
    )
...
async def get_available_vehicles(
    ...
    vehicles, total, total_pages = await service.get_available_vehicles(page, perPage)
...
async def update_vehicle(
    ...
    return SuccessResponse(data=vehicle, message="Vehicle updated successfully")
...
async def delete_vehicle(
    ...
    return SuccessResponse(data=result, message="Vehicle deleted successfully")
