...
// Synthetic "Current Year P/(L)" row inside Equity — a subtle lapis tint
// (not a phase colour; this is a computed/live figure, not a status) marks
// it as distinct from posted account rows.
const PLRow = styled.tr`
  border-bottom: 1px solid ${({ theme }) => theme.colors.line};
  background: rgba(107, 138, 224, 0.08);
`;
...
// Accounting-identity check row — the one place on this statement where
// debit/credit polarity applies: imbalanced reads bright.coral (spec
// judgment call, same convention as the P&L pages in this shard).
const IdentityRow = styled.tr<IdentityRowProps>`
  border-top: 3px double ${({ theme }) => theme.colors.line};
  background: ${({ $imbalanced }) => ($imbalanced ? 'rgba(240, 138, 112, 0.12)' : 'transparent')};
`;
...
const IdentityAmountCell = styled.td<IdentityAmountCellProps>`
  ...
  font-family: ${({ theme }) => theme.typography.fontFamily.mono};
  text-align: right;
  color: ${({ $imbalanced, theme }) => ($imbalanced ? theme.colors.bright.coral : 'inherit')};
  white-space: nowrap;
`;
