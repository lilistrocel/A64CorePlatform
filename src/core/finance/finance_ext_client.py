"""
Core Finance — Finance-Ext HTTP Client (shared helper)

Provides the ``get_item_finance_ext``, ``get_customer_finance_ext``, and
``get_tax_percent`` coroutines used by multiple service modules across the
ops backend (sales, purchasing).  Extracted from
``src/modules/sales/services/_finance_ext_client.py`` on 2026-06-10 as part
of T-200.22b so that both sales and purchasing can use the same HTTP client
without duplicating the implementation.  Pattern matches T-200.22a's
``src/core/documents/chain_reconciler.py`` extraction.
...
::
    ...
    from src.core.finance.finance_ext_client import get_item_finance_ext, get_tax_percent
...
async def get_customer_finance_ext(
    customer_id: str,
    org_id: str,
    auth_token: Optional[str],
) -> Dict[str, Any]:
    ...
    Returns:
        Dict of the customer finance extension fields (camelCase, matching the
        finance service's CustomerFinanceExtResponse schema).
    ...
    try:
        ...
        raise ValueError(
            f"Finance service unreachable when looking up customer '{customer_id}': {exc}. "
            "Ensure FINANCE_SERVICE_URL is set and the finance service is running."
        ) from exc

    if resp.status_code == 404:
        raise ValueError(
            f"Customer '{customer_id}' has no customer_finance_ext record in org '{org_id}'. "
            "Configure the customer's finance extension before invoicing."
        )
