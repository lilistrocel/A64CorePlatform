...
class VehicleService:
    ...
    async def create_vehicle(
        self, vehicle_data: VehicleCreate, created_by: UUID
    ) -> Vehicle:
        """
    ...
    async def get_all_vehicles(
        ...
        vehicles, total = await self.repository.get_all(
            skip, per_page, status, type, ownership
        )
    ...
    async def update_vehicle(
        ...
        if (
            update_data.licensePlate is not None
            and not update_data.licensePlate.strip()
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
