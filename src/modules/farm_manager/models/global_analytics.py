...
class FarmSummaryItem(BaseModel):
    ...
    currentUtilization: float = Field(
        0.0, ge=0, le=100, description="Current utilization (%)"
    )
...
class GlobalAggregatedMetrics(BaseModel):
    ...
    totalActivePlantings: int = Field(
        0, description="Total active plantings across all farms"
    )
    totalYieldKg: float = Field(
        0.0, description="Total actual yield across all farms (kg)"
    )
    avgYieldEfficiencyAcrossFarms: float = Field(
        0.0, description="Weighted average yield efficiency (%)"
    )
    avgPerformanceScore: float = Field(
        0.0, ge=0, le=100, description="Average performance score across all farms"
    )
    totalCapacity: int = Field(
        0, description="Total capacity (maxPlants) across all farms"
    )
    avgUtilization: float = Field(
        0.0, ge=0, le=100, description="Average utilization across all farms (%)"
    )
    totalPredictedYieldKg: float = Field(
        0.0, description="Total predicted yield across all farms (kg)"
    )
...
class GlobalYieldTimeline(BaseModel):
    ...
    totalYieldKg: float = Field(
        0.0, description="Total yield harvested across all farms (kg)"
    )
    harvestCount: int = Field(0, description="Total number of harvest events")
    farmCount: int = Field(
        0, description="Number of farms that had harvests on this date"
    )
...
class GlobalPerformanceInsights(BaseModel):
    ...
    topPerformingFarms: List[FarmSummaryItem] = Field(
        default_factory=list, description="Top 5 performing farms by performance score"
    )
...
class GlobalAnalyticsResponse(BaseModel):
    ...
    generatedAt: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp of report generation"
    )

    aggregatedMetrics: GlobalAggregatedMetrics
    stateBreakdown: GlobalStateBreakdown
    farmSummaries: List[FarmSummaryItem] = Field(
        default_factory=list, description="Summary statistics for each farm"
    )
