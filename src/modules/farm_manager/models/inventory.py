...
def convert_to_base_unit(
    quantity: float, display_unit: str, category: InputCategory
) -> float:
    """
...
def convert_from_base_unit(
    base_quantity: float, display_unit: str, category: InputCategory
) -> float:
    """
...
class Product(BaseModel):
    ...
    productId: UUID = Field(
        default_factory=uuid4, description="Unique product identifier"
    )
    ...
    unit: str = Field(
        ...
        min_length=1,
        max_length=20,
        description="Default display unit (kg, L, units)",
    )
    baseUnit: BaseUnit = Field(
        ..., description="Base unit for calculations (mg, ml, unit)"
    )
    conversionFactor: float = Field(
        ..., gt=0, description="Convert display unit to base unit"
    )

    # Optional metadata
    brand: Optional[str] = Field(None, max_length=100, description="Brand/manufacturer")
    sku: Optional[str] = Field(
        None, max_length=50, description="Stock keeping unit/product code"
    )
...
class HarvestInventoryBase(BaseModel):
    ...
    farmId: UUID | None = Field(
        default=None,
        description="Farm this inventory belongs to (null for organization inventory)",
    )
    organizationId: UUID = Field(
    ...
    productType: HarvestProductType = Field(
        HarvestProductType.FRESH, description="Type of product"
    )
    variety: Optional[str] = Field(
        None, max_length=100, description="Plant variety/cultivar"
    )
    ...
    unit: str = Field(
        ...
        min_length=1,
        max_length=20,
        description="Unit of measurement (kg, units, bunches)",
    )

    # Quality and grading
    qualityGrade: QualityGrade = Field(
        QualityGrade.GRADE_A, description="Quality grade"
    )

    # Dates
    harvestDate: datetime = Field(..., description="When the product was harvested")
    expiryDate: Optional[datetime] = Field(
        None, description="Expected expiry/best before date"
    )

    # Storage
    storageLocation: Optional[str] = Field(
        None, max_length=200, description="Where the product is stored"
    )
    storageConditions: Optional[str] = Field(
        None, max_length=500, description="Storage conditions (temp, humidity)"
    )
...
class HarvestInventoryCreate(HarvestInventoryBase):
    ...
    quantity: float = Field(
        ..., gt=0, description="Initial quantity must be greater than 0"
    )
...
class HarvestInventory(HarvestInventoryBase):
    ...
    inventoryScope: InventoryScope = Field(
        ..., description="Scope level: organization or farm"
    )
    ...
    originalQuantity: float = Field(
        ..., ge=0, description="Original recorded quantity at creation (immutable)"
    )

    # Source tracking (links to block harvest if auto-created)
    sourceHarvestId: Optional[UUID] = Field(
        None, description="Reference to block harvest (if auto-created from harvest)"
    )

    # Farming year (calculated from harvestDate)
    farmingYear: Optional[int] = Field(
        None, description="Farming year derived from harvest date"
    )
...
class InputInventoryBase(BaseModel):
    ...
    unit: str = Field(
        ...
        min_length=1,
        max_length=20,
        description="Display unit (kg, g, L, ml, etc.)",
    )
    minimumStock: float = Field(
        0.0, ge=0, description="Minimum stock level in display units"
    )

    # Dates
    purchaseDate: Optional[datetime] = Field(
        None, description="When the item was purchased"
    )
    expiryDate: Optional[datetime] = Field(None, description="Expiry date")

    # Storage
    storageLocation: Optional[str] = Field(
        None, max_length=200, description="Storage location"
    )
    ...
    activeIngredients: Optional[str] = Field(
        None, max_length=500, description="Active ingredients"
    )
    concentration: Optional[str] = Field(
        None, max_length=100, description="Concentration/strength"
    )
    applicationRate: Optional[str] = Field(
        None, max_length=200, description="Recommended application rate"
    )
    safetyNotes: Optional[str] = Field(
        None, max_length=1000, description="Safety precautions"
    )
...
class InputInventory(InputInventoryBase):
    ...
    baseUnit: BaseUnit = Field(
        ..., description="Base unit for calculations (mg, ml, or unit)"
    )
    baseQuantity: float = Field(
        ..., ge=0, description="Quantity in base units for precise calculations"
    )
    baseMinimumStock: float = Field(
        0.0, ge=0, description="Minimum stock in base units"
    )
    ...
    transferHistory: List[dict] = Field(
        default_factory=list, description="History of transfers"
    )
    ...
    lastUsedAt: Optional[datetime] = Field(
        None, description="Last time this item was used"
    )
...
class AssetInventoryBase(BaseModel):
    ...
    farmId: UUID | None = Field(
        default=None,
        description="Farm this asset belongs to (null for organization inventory)",
    )
    ...
    assetTag: Optional[str] = Field(
        None, max_length=50, description="Internal asset tag/ID"
    )
    serialNumber: Optional[str] = Field(
        None, max_length=100, description="Manufacturer serial number"
    )
    ...
    year: Optional[int] = Field(
        None, ge=1900, le=2100, description="Year of manufacture"
    )

    # Status
    status: AssetStatus = Field(AssetStatus.OPERATIONAL, description="Current status")
    condition: Optional[str] = Field(
        None, max_length=200, description="Current condition description"
    )

    # Location
    location: Optional[str] = Field(
        None, max_length=200, description="Current location"
    )
    assignedTo: Optional[str] = Field(
        None, max_length=200, description="Person/team assigned to"
    )
    ...
    currentValue: Optional[float] = Field(
        None, ge=0, description="Current estimated value"
    )
    currency: str = Field("AED", max_length=3, description="Currency code")

    # Maintenance
    lastMaintenanceDate: Optional[datetime] = Field(
        None, description="Last maintenance date"
    )
    nextMaintenanceDate: Optional[datetime] = Field(
        None, description="Next scheduled maintenance"
    )
    maintenanceNotes: Optional[str] = Field(
        None, max_length=1000, description="Maintenance history/notes"
    )

    # Specifications
    specifications: Optional[str] = Field(
        None, max_length=2000, description="Technical specifications"
    )

    # Documentation
    warrantyExpiry: Optional[datetime] = Field(None, description="Warranty expiry date")
    documentationUrl: Optional[str] = Field(
        None, max_length=500, description="Link to documentation"
    )
...
class AssetInventory(AssetInventoryBase):
    ...
    # Maintenance alerts
    maintenanceOverdue: bool = Field(
        False, description="True if maintenance is overdue"
    )

    # NEW: Asset allocation tracking (for shared assets)
    currentAllocation: Optional[dict] = Field(
        None, description="Current allocation information"
    )
    allocationHistory: List[dict] = Field(
        default_factory=list, description="History of allocations"
    )
...
class InventoryMovement(BaseModel):
    ...
    movementId: UUID = Field(
        default_factory=uuid4, description="Unique movement identifier"
    )
    ...
    organizationId: UUID = Field(
        ..., description="Organization this movement belongs to"
    )

    # NEW: Scope tracking (for transfers)
    fromScope: Optional[InventoryScope] = Field(
        None, description="Source scope (for transfers)"
    )
    toScope: Optional[InventoryScope] = Field(
        None, description="Destination scope (for transfers)"
    )
    fromFarmId: Optional[UUID] = Field(None, description="Source farm (for transfers)")
    toFarmId: Optional[UUID] = Field(
        None, description="Destination farm (for transfers)"
    )

    reason: Optional[str] = Field(
        None, max_length=500, description="Reason for movement"
    )
    referenceId: Optional[str] = Field(
        None, max_length=100, description="Reference to related record (order, task)"
    )
...
class InventorySummary(BaseModel):
    ...
    harvestInventory: dict = Field(
        default_factory=dict, description="Harvest inventory summary"
    )
    inputInventory: dict = Field(
        default_factory=dict, description="Input inventory summary"
    )
    assetInventory: dict = Field(
        default_factory=dict, description="Asset inventory summary"
    )
    wasteInventory: dict = Field(
        default_factory=dict, description="Waste inventory summary"
    )

    totalHarvestValue: float = Field(
        0.0, description="Total value of harvest inventory"
    )
...
class TransferRequest(BaseModel):
    ...
    fromFarmId: Optional[UUID] = Field(
        None, description="Source farm ID (if from farm)"
    )
    toFarmId: Optional[UUID] = Field(
        None, description="Destination farm ID (if to farm)"
    )

    quantity: float = Field(
        ..., gt=0, description="Quantity to transfer in display units"
    )
    unit: str = Field(..., description="Unit of quantity")

    reason: str = Field(
        ..., min_length=1, max_length=500, description="Reason for transfer"
    )
...
class TransferResponse(BaseModel):
    ...
    destinationInventory: Optional[dict] = Field(
        None, description="Destination inventory item (if created)"
    )
...
class TransferRecord(BaseModel):
    ...
    quantityTransferred: float = Field(
        ..., description="Quantity transferred in base units"
    )
...
class WasteInventoryBase(BaseModel):
    ...
    sourceInventoryId: Optional[UUID] = Field(
        None, description="Original inventory item ID"
    )
    sourceOrderId: Optional[UUID] = Field(
        None, description="Sales order ID (for returns)"
    )
    sourceReturnId: Optional[UUID] = Field(
        None, description="Return order ID (for returns)"
    )
    ...
    # Product info
    plantName: str = Field(
        ..., min_length=1, max_length=200, description="Product/plant name"
    )
    ...
    wasteDate: datetime = Field(
        default_factory=datetime.utcnow, description="Date item became waste"
    )

    # Disposal
    disposalMethod: DisposalMethod = Field(
        DisposalMethod.PENDING, description="Disposal method"
    )
    disposalDate: Optional[datetime] = Field(None, description="Date of disposal")
    disposalNotes: Optional[str] = Field(
        None, max_length=500, description="Disposal notes"
    )

    # Value tracking (for reporting)
    estimatedValue: Optional[float] = Field(
        None, ge=0, description="Estimated value of waste"
    )
    currency: str = Field("AED", max_length=3, description="Currency code")
...
class WasteInventoryCreate(WasteInventoryBase):
    ...
    organizationId: Optional[UUID] = Field(
        None, description="Organization (set automatically from auth)"
    )
...
class MoveToWasteRequest(BaseModel):
    ...
    inventoryType: InventoryType = Field(
        InventoryType.HARVEST, description="Type of inventory"
    )
    quantity: float = Field(..., gt=0, description="Quantity to move to waste")
    wasteReason: str = Field(..., max_length=500, description="Reason for waste")
    sourceType: WasteSourceType = Field(
        WasteSourceType.EXPIRED, description="Source type"
    )
...
class WasteSummary(BaseModel):
    ...
    bySourceType: dict = Field(
        default_factory=dict, description="Breakdown by source type"
    )
    byDisposalMethod: dict = Field(
        default_factory=dict, description="Breakdown by disposal method"
    )
...
class ReviveHarvestRequest(BaseModel):
    ...
    notes: Optional[str] = Field(
        None, max_length=1000, description="Optional notes for the audit trail"
    )
...
class MarkWasteRequest(BaseModel):
    ...
    wasteReason: Optional[str] = Field(
        None, max_length=500, description="Reason for waste"
    )
...
class ReturnedInventoryBase(BaseModel):
    ...
    plantDataId: UUID = Field(
        ..., description="Reference to plant data catalogue entry"
    )
    plantName: str = Field(
        ..., min_length=1, max_length=200, description="Product name"
    )
    productType: HarvestProductType = Field(
        HarvestProductType.FRESH, description="Type of product (fresh, processed, etc.)"
    )
    variety: Optional[str] = Field(
        None, max_length=100, description="Plant variety / cultivar"
    )
    qualityGrade: QualityGrade = Field(
        QualityGrade.GRADE_A, description="Quality grade on receipt"
    )
    ...
    returnDate: datetime = Field(
        ..., description="When this return was recorded in the system"
    )

    # Source traceability
    sourceOrderId: UUID = Field(
        ..., description="Sales order that generated this return"
    )
    ...
    sourceBlockId: Optional[UUID] = Field(
        None, description="Block the goods originally grew in"
    )

    returnReason: Optional[str] = Field(
        None, max_length=500, description="Reason given for the return"
    )
    conditionNotes: Optional[str] = Field(
        None, max_length=1000, description="Condition of goods on receipt"
    )
...
class ReturnedInventoryCreate(ReturnedInventoryBase):
    ...
    organizationId: Optional[UUID] = Field(
        None, description="Organisation ID — set automatically from auth context"
    )
...
class ReturnedInventory(ReturnedInventoryBase):
    ...
    reservedQuantity: float = Field(
        0.0, ge=0, description="Quantity reserved for pending orders"
    )
    availableQuantity: float = Field(
        ..., ge=0, description="Quantity available for re-allocation"
    )

    farmingYear: Optional[int] = Field(
        None, description="Farming year derived from harvestDate"
    )
    divisionId: Optional[str] = Field(None, description="Division scope")
