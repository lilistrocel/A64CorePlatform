...
class ReturnService:
    ...
    async def create_return(
        self, return_data: ReturnOrderCreate, created_by: UUID
    ) -> ReturnOrder:
        """
        ...
        order_doc = await db.sales_orders.find_one(
            {"orderId": str(return_data.orderId)}
        )
        if not order_doc:
            raise HTTPException(
        ...
        valid_statuses = [
            ...
            SalesOrderStatus.IN_TRANSIT.value,
        ]
        ...
        logger.info(
            f"Return order created: {return_order.returnId} for order {return_data.orderId}"
        )
        return return_order
    ...
    async def update_return(
        self, return_id: UUID, update_data: ReturnOrderUpdate
    ) -> ReturnOrder:
        ...
        await self._get_collection().update_one(
            {"returnId": str(return_id)}, {"$set": update_dict}
        )
    ...
    async def process_return(
        self, process_request: ProcessReturnRequest, processed_by: UUID
    ) -> ProcessReturnResponse:
        """
        ...
        await self._get_collection().update_one(
            ...
            {
                "$set": {
                    "status": ReturnStatus.PROCESSING.value,
        ...
        for item in return_order.items:
            ...
            if process_request.itemOverrides:
                for override in process_request.itemOverrides:
                    if str(override.get("orderItemId")) == str(
                        item.get("orderItemId") or item.orderItemId
                    ):
            ...
            return_to_inventory = (
                item_override.get("returnToInventory", item.returnToInventory)
                if item_override
                else item.returnToInventory
            )
            new_grade = (
                item_override.get("newGrade", item.newGrade)
                if item_override
                else item.newGrade
            )
            condition = (
                item.condition
                if isinstance(item.condition, str)
                else item.condition.value
            )

            # Items in poor condition always go to waste
            if condition in [
                ReturnCondition.SPOILED.value,
                ReturnCondition.CONTAMINATED.value,
                "spoiled",
                "contaminated",
            ]:
            ...
            if return_to_inventory:
                ...
                inventory_update = await self._return_to_inventory(
                    farm_db_instance, item, new_grade, return_order, processed_by
                )
                if inventory_update:
                ...
                waste_record = await self._create_waste_record(
                    farm_db_instance, item, return_order, processed_by
                )
                if waste_record:
        ...
        order_doc = await sales_db_instance.sales_orders.find_one(
            {"orderId": str(return_order.orderId)}
        )
        if order_doc:
            total_ordered = sum(
                item.get("quantity", 0) for item in order_doc.get("items", [])
            )
        ...
        logger.info(
            f"Return order {process_request.returnId} processed: {len(inventory_updates)} inventory updates, {len(waste_created)} waste records"
        )
    ...
    async def _return_to_inventory(
        ...
        returned_qty = item.get("returnedQuantity") or getattr(
            item, "returnedQuantity", 0
        )
        original_grade = item.get("originalGrade") or getattr(
            item, "originalGrade", None
        )
    ...
    async def _create_waste_record(
        self, db, item, return_order: ReturnOrder, processed_by: UUID
    ) -> Optional[dict]:
        ...
        waste_record = {
            ...
            "wasteReason": (
                ...
                else f"{reason.value if hasattr(reason, 'value') else reason}: {condition.value if hasattr(condition, 'value') else condition}"
            ),
