...
api_router.include_router(
    admin.router, tags=["Admin"]
)  # Admin routes have /admin prefix in router
api_router.include_router(
    modules.router, tags=["Module Management"]
)  # Module routes have /modules prefix in router
api_router.include_router(
    dashboard.router, tags=["Dashboard"]
)  # Dashboard routes have /dashboard prefix in router

# Include multi-industry framework routes
api_router.include_router(
    organizations.router, tags=["Organizations"]
)  # /organizations prefix in router
api_router.include_router(
    divisions.router, tags=["Divisions"]
)  # /divisions prefix in router
api_router.include_router(
    industries.router, tags=["Industries"]
)  # /industries prefix in router

# Wave 0 — capability discovery (per-tenant module status)
# Endpoints at /api/v1/system/*
api_router.include_router(system.router, tags=["System"])

# Include AI analytics routes
api_router.include_router(
    ai_chat.router, tags=["AI Analytics"]
)  # AI routes at /api/v1/ai/*
