...
class GrowthCycleDuration(BaseModel):
    ...
    floweringDays: int = Field(
        0, ge=0, description="Days of flowering stage (0 if not applicable)"
    )
    fruitingDays: int = Field(
        0, ge=0, description="Days of fruiting stage (0 if not applicable)"
    )
    harvestDurationDays: int = Field(
        0, ge=0, description="Duration of harvest period in days"
    )
    totalCycleDays: int = Field(
        ..., gt=0, description="Total growth cycle from seed to harvest"
    )
...
class YieldInfo(BaseModel):
    ...
    yieldUnit: str = Field(
        ..., description="Unit of measurement (kg, lbs, units, etc.)"
    )
    ...
    expectedWastePercentage: float = Field(
        0, ge=0, le=100, description="Expected waste/loss percentage"
    )
        ...
        if info.data:
            ...
            if min_temp is not None and max_temp is not None:
                if not (min_temp <= v <= max_temp):
                    raise ValueError(
                        f"Optimal temperature must be between {min_temp} and {max_temp}"
                    )
        return v
...
class HumidityRange(BaseModel):
    ...
    minPercentage: float = Field(
        ..., ge=0, le=100, description="Minimum humidity percentage"
    )
    maxPercentage: float = Field(
        ..., ge=0, le=100, description="Maximum humidity percentage"
    )
    optimalPercentage: float = Field(
        ..., ge=0, le=100, description="Optimal humidity percentage"
    )
...
class EnvironmentalRequirements(BaseModel):
    ...
    co2RequirementPpm: Optional[int] = Field(
        None, ge=0, description="CO2 requirement in PPM (for greenhouse/indoor)"
    )
    airCirculation: Optional[str] = Field(
        None, description="Air circulation requirements (e.g., 'moderate', 'high')"
    )
...
class WateringRequirements(BaseModel):
    ...
    waterType: WaterTypeEnum = Field(
        WaterTypeEnum.TAP, description="Preferred water type"
    )
    amountPerPlantLiters: Optional[float] = Field(
        None, gt=0, description="Water amount per plant per watering (liters)"
    )
    droughtTolerance: ToleranceLevelEnum = Field(
        ToleranceLevelEnum.MEDIUM, description="Drought tolerance level"
    )
...
class SoilRequirements(BaseModel):
    ...
    nutrientsRecommendations: Optional[str] = Field(
        None, description="Soil nutrient recommendations (N, P, K, Ca, Mg, etc.)"
    )
    ecRangeMs: Optional[str] = Field(
        None, description="EC range in mS/cm (for hydroponics, e.g., '1.5-2.5')"
    )
    tdsRangePpm: Optional[str] = Field(
        None, description="TDS range in PPM (for hydroponics, e.g., '1000-1500')"
    )
...
class LightRequirements(BaseModel):
    ...
    minHoursDaily: float = Field(
        ..., ge=0, le=24, description="Minimum daily light hours"
    )
    maxHoursDaily: float = Field(
        ..., ge=0, le=24, description="Maximum daily light hours"
    )
    optimalHoursDaily: float = Field(
        ..., ge=0, le=24, description="Optimal daily light hours"
    )
    intensityLux: Optional[int] = Field(
        None, ge=0, description="Light intensity in Lux (for indoor)"
    )
    intensityPpfd: Optional[int] = Field(
        None, ge=0, description="Light intensity in PPFD (for indoor)"
    )
    photoperiodSensitive: bool = Field(
        False, description="Whether plant is photoperiod sensitive"
    )
...
class QualityGrade(BaseModel):
    ...
    gradeName: str = Field(
        ..., description="Grade name (A, B, C, Premium, Standard, etc.)"
    )
    sizeRequirements: Optional[str] = Field(
        None, description="Size requirements for this grade"
    )
    colorRequirements: Optional[str] = Field(
        None, description="Color requirements for this grade"
    )
    defectTolerance: Optional[str] = Field(
        None, description="Allowable defect tolerance"
    )
    otherCriteria: Optional[str] = Field(None, description="Other quality criteria")
    priceMultiplier: Optional[float] = Field(
        None, ge=0, description="Price multiplier vs base grade (1.0 = base)"
    )
...
class EconomicsAndLabor(BaseModel):
    ...
    averageMarketValuePerKg: Optional[float] = Field(
        None, gt=0, description="Average market value per kg"
    )
    currency: str = Field("USD", description="Currency code (USD, EUR, etc.)")
    totalManHoursPerPlant: float = Field(
        ..., ge=0, description="Total labor hours per plant (full lifecycle)"
    )
    plantingHours: Optional[float] = Field(
        None, ge=0, description="Labor hours for planting per plant"
    )
    maintenanceHours: Optional[float] = Field(
        None, ge=0, description="Labor hours for maintenance per plant"
    )
    harvestingHours: Optional[float] = Field(
        None, ge=0, description="Labor hours for harvesting per plant"
    )
...
class SpacingRequirements(BaseModel):
    ...
    betweenPlantsCm: float = Field(
        ..., gt=0, description="Spacing between plants in centimeters"
    )
    betweenRowsCm: float = Field(
        ..., gt=0, description="Spacing between rows in centimeters"
    )
    plantsPerSquareMeter: Optional[float] = Field(
        None, gt=0, description="Calculated plants per square meter"
    )
...
class AdditionalInformation(BaseModel):
    ...
    supportRequirements: SupportTypeEnum = Field(
        SupportTypeEnum.NONE, description="Support structure needed"
    )
    companionPlants: Optional[List[str]] = Field(
        None, description="Beneficial companion plants"
    )
    incompatiblePlants: Optional[List[str]] = Field(
        None, description="Plants to avoid planting nearby"
    )
...
class FertigationIngredient(BaseModel):
    ...
    category: IngredientCategoryEnum = Field(
        IngredientCategoryEnum.OTHER, description="Ingredient category for grouping"
    )
    dosagePerPoint: float = Field(
        ..., ge=0, description="Dosage per planting point per application"
    )
...
class CustomApplication(BaseModel):
    ...
    ingredients: List[FertigationIngredient] = Field(
        ..., description="Ingredients to apply on this day"
    )
...
class FertigationRule(BaseModel):
    ...
    name: str = Field(
        ..., description="Human-readable rule name (e.g., 'Weekly Base Feed')"
    )
    type: FertigationRuleTypeEnum = Field(
        ..., description="Rule type: 'interval' (regular) or 'custom' (day-by-day)"
    )

    # For interval rules
    frequencyDays: Optional[int] = Field(
        None, gt=0, description="Apply every N days (interval type only)"
    )
    activeDayStart: Optional[int] = Field(
        None,
        ge=0,
        description="First application day relative to planting (interval type only)",
    )
    activeDayEnd: Optional[int] = Field(
        None,
        ge=0,
        description="Last application day relative to planting (interval type only)",
    )
    ingredients: Optional[List[FertigationIngredient]] = Field(
        None, description="Ingredients applied per this rule (interval type only)"
    )

    # For custom rules
    applications: Optional[List[CustomApplication]] = Field(
        None, description="Explicit day-by-day applications (custom type only)"
    )
...
class FertigationCard(BaseModel):
    ...
    cardName: str = Field(
        ..., description="Card label (e.g., 'Full Cycle', 'Vegetative Growth')"
    )
    growthStage: str = Field(
        ...,
        description="Growth stage this card applies to (general, germination, vegetative, flowering, fruiting, harvest)",
    )
    dayStart: int = Field(
        ..., ge=0, description="First day this card is active (relative to planting)"
    )
    dayEnd: int = Field(
        ..., ge=0, description="Last day this card is active (relative to planting)"
    )
    rules: List[FertigationRule] = Field(
        default_factory=list, description="Application rules for this card"
    )
    notes: Optional[str] = Field(
        None, description="Additional notes about this fertigation card"
    )
...
class FertigationSchedule(BaseModel):
    ...
    cards: List[FertigationCard] = Field(
        default_factory=list, description="Fertigation cards by growth stage"
    )
    totalFertilizationDays: int = Field(
        0, ge=0, description="Total days requiring fertigation"
    )
    source: str = Field(
        "manual", description="Data source: 'legacy_migration' or 'manual'"
    )
...
class PlantDataEnhancedBase(BaseModel):
    ...
    farmTypeCompatibility: List[FarmTypeEnum] = Field(
        ..., description="Compatible farm types"
    )

    # ===== 2. Growth Cycle Durations =====
    growthCycle: GrowthCycleDuration = Field(
        ..., description="Detailed growth cycle breakdown"
    )
    ...
    environmentalRequirements: Optional[EnvironmentalRequirements] = Field(
        None, description="Environmental conditions"
    )

    # ===== 5. Watering Requirements =====
    wateringRequirements: Optional[WateringRequirements] = Field(
        None, description="Watering specifications"
    )

    # ===== 6. pH & Soil Requirements =====
    soilRequirements: Optional[SoilRequirements] = Field(
        None, description="Soil and pH requirements"
    )

    # ===== 7. Disease & Pest Management =====
    diseasesAndPests: List[DiseaseOrPest] = Field(
        default_factory=list, description="Known diseases and pests"
    )

    # ===== 8. Light Requirements =====
    lightRequirements: Optional[LightRequirements] = Field(
        None, description="Light and photoperiod requirements"
    )

    # ===== 9. Grading Standards =====
    gradingStandards: List[QualityGrade] = Field(
        default_factory=list, description="Quality grading standards"
    )

    # ===== 10. Economics & Labor =====
    economicsAndLabor: Optional[EconomicsAndLabor] = Field(
        None, description="Economic and labor information"
    )

    # ===== 11. Additional Information =====
    additionalInfo: Optional[AdditionalInformation] = Field(
        None, description="Additional agronomic details"
    )
    ...
    tags: Optional[List[str]] = Field(
        None, description="Search tags (vegetable, fruit, summer, etc.)"
    )
...
class PlantDataEnhanced(PlantDataEnhancedBase):
    ...
    plantDataId: UUID = Field(
        default_factory=uuid4, description="Unique plant data identifier (UUID v4)"
    )

    # Versioning (for freezing data when used in planting plans)
    dataVersion: int = Field(
        1, description="Data version number (increment on updates)"
    )
...
class PlantDataLegacy(BaseModel):
    ...
    def from_enhanced(cls, enhanced: PlantDataEnhanced) -> "PlantDataLegacy":
        ...
        return cls(
            ...
            plantType=(
                enhanced.farmTypeCompatibility[0]
