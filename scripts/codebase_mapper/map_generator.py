...
def gen_index(db) -> str:
    ...
    lines = [
        ...
        f"- **Infrastructure:** Docker Compose, Nginx, business modules (see Module Directory below — hard-coding a count here only goes stale)",
    ...
    map_descriptions = {
        ...
        "module-map.md": "Every backend module under `src/modules/` and its nodes by layer, plus cross-module dependencies (see the Module Directory below for the authoritative list)",
    ...
    lines += [
        ...
        f"|--------|----------|---------|",
        f"| `farm_manager` | `src/modules/farm_manager/` | Farm blocks, harvests, plant data, analytics (industry: vegetable_fruits) |",
        f"| `mushroom_manager` | `src/modules/mushroom_manager/` | Facilities, growing rooms, substrate, flush harvests, strain growing-profiles (industry: mushroom) |",
        f"| `genetics` | `src/modules/genetics/` | Genetics Repo — lines, accessions with G/F generations, propagation events, medium recipes, observations (industry: all, shared) |",
        ...
        f"| `purchasing` | `src/modules/purchasing/` | PR→PO→GR→AP chain, vendors, purchase items, payment terms, approvals |",
        f"| `finance` | `src/modules/finance/` | Operational P&L analytics from MongoDB, mounted at `/api/v1/operations`. NOT the statutory GL — that is the MySQL microservice at `services/finance/` (`/api/v1/finance/*`, profile-gated), which has no mapping task and no nodes in this graph. |",
        ...
        f"| `ai_assistant` | `src/modules/ai_assistant/` | Claude assistant: conversations, tool executor, cost tracking. Not a plugin — mounted from `src/api/routes.py` at `/api/v1/ai` |",
        f"| `attachments` | `src/modules/attachments/` | Document attachments, pluggable storage, range requests. Not a plugin — mounted from `src/api/routes.py` at `/api/v1/attachments` |",
        f"| `protocols` | `src/modules/protocols/` | Versioned SOPs, approve/retire lifecycle, scope tags, version pinning |",
        f"| `finance_bridge` | `src/modules/finance_bridge/` | Outbox writer bridging ops events to the finance microservice |",
        f"",
