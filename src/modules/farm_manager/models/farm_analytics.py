...
class AggregatedMetrics(BaseModel):
    ...
    activePlantings: int = Field(
        0, description="Blocks currently in growing/fruiting/harvesting states"
    )
    totalYieldKg: float = Field(
        0.0, description="Total actual yield across all blocks (kg)"
    )
    avgYieldEfficiency: float = Field(
        0.0, description="Weighted average yield efficiency (%)"
    )
    overallPerformanceScore: float = Field(
        0.0, ge=0, le=100, description="Average performance score across blocks"
    )
    totalCapacity: int = Field(0, description="Sum of maxPlants across all blocks")
    currentUtilization: float = Field(
        0.0, ge=0, le=100, description="Current plants / capacity (%)"
    )
    predictedYieldKg: float = Field(
        0.0, description="Total predicted yield across all active plantings (kg)"
    )
...
class StateBreakdownItem(BaseModel):
    ...
    blockIds: List[UUID] = Field(
        default_factory=list, description="List of block IDs in this state"
    )
    avgDaysInState: Optional[float] = Field(
        None, description="Average days blocks have been in this state"
    )
...
class BlockComparisonItem(BaseModel):
    ...
    performanceScore: float = Field(
        0.0, ge=0, le=100, description="Overall performance score"
    )
    daysInCycle: Optional[int] = Field(None, description="Days since planting")
    taskCompletionRate: float = Field(
        0.0, ge=0, le=100, description="Task completion rate (%)"
    )
...
class YieldTimelinePoint(BaseModel):
    ...
    totalYieldKg: float = Field(
        0.0, description="Total yield harvested on this date (kg)"
    )
    harvestCount: int = Field(0, description="Number of harvest events")
    blockIds: List[UUID] = Field(
        default_factory=list, description="Blocks that had harvests"
    )
...
class HistoricalTrends(BaseModel):
    ...
    yieldTimeline: List[YieldTimelinePoint] = Field(
        default_factory=list, description="Daily/weekly yield totals over period"
    )
    stateTransitions: List[StateTransitionEvent] = Field(
        default_factory=list, description="Recent state transitions across farm"
    )
    performanceTrend: str = Field(
        "insufficient_data",
        description="Overall trend: improving, stable, declining, insufficient_data",
    )
    avgHarvestsPerWeek: float = Field(
        0.0, description="Average harvest events per week"
    )
...
class FarmAnalyticsResponse(BaseModel):
    ...
    blockComparison: List[BlockComparisonItem] = Field(
        default_factory=list, description="Comparison data for all blocks"
    )
