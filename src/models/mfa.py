...
class MFASetupResponse(BaseModel):
    ...
    provisioningUri: str = Field(
        ..., description="URI for authenticator app (otpauth://)"
    )
    secret: str = Field(
        ..., description="Base32 secret for manual entry (shown only once)"
    )
    method: MFAMethod = MFAMethod.TOTP
    qrCodeDataUrl: Optional[str] = Field(
        None, description="Base64 data URL for QR code image"
    )
...
class MFAInDB(BaseModel):
    ...
    backupCodesHash: Optional[list[str]] = Field(
        None, description="Hashed backup codes"
    )
    createdAt: datetime
    updatedAt: datetime
    enabledAt: Optional[datetime] = None
...
class UserMFA(BaseModel):
    ...
    isEnabled: bool = Field(
        default=False, description="Whether MFA is currently enabled"
    )
    method: MFAMethod = Field(
        default=MFAMethod.TOTP, description="MFA method (currently only TOTP)"
    )
    ...
    disabledAt: Optional[datetime] = Field(
        None, description="When MFA was last disabled"
    )

    # Usage tracking
    lastUsedAt: Optional[datetime] = Field(
        None, description="Last successful MFA verification"
    )
    lastUsedCounter: int = Field(
        default=0, description="Last TOTP counter for replay protection"
    )

    # Security features
    failedAttempts: int = Field(
        default=0, description="Consecutive failed verification attempts"
    )
...
class MFABackupCode(BaseModel):
    ...
    expiresAt: Optional[datetime] = Field(
        None, description="TTL expiry for used codes (90 days after use)"
    )
...
class MFAAuditLog(BaseModel):
    ...
    userId: str = Field(
        ..., description="User ID who performed/was affected by the action"
    )
    ...
    details: Optional[dict] = Field(
        None, description="Additional action-specific details"
    )
    performedBy: Optional[str] = Field(
        None, description="Admin user ID if action was performed by admin"
    )
