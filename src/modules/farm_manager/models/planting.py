...
class PlantingItem(BaseModel):
    ...
    plantDataSnapshot: Dict = Field(
        ..., description="Snapshot of plant data at planting time"
    )
...
class Planting(PlantingBase):
    ...
    plantingId: UUID = Field(
        default_factory=uuid4, description="Unique planting identifier"
    )
    ...
    plannedAt: datetime = Field(
        default_factory=datetime.utcnow, description="When planning was created"
    )
    ...
    estimatedHarvestStartDate: Optional[datetime] = Field(
        None, description="Estimated harvest start"
    )
    estimatedHarvestEndDate: Optional[datetime] = Field(
        None, description="Estimated harvest end"
    )
    ...
    # Status
    status: str = Field(
        "planned", description="Status: planned, planted, harvesting, completed"
    )
