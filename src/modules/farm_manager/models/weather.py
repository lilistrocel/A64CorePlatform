...
class SolarData(BaseModel):
    ...
    dhi: Optional[float] = Field(
        None, description="Diffuse Horizontal Irradiance (W/m²)"
    )

    # Sun position
    sunElevation: Optional[float] = Field(
        None, description="Sun elevation angle (degrees)"
    )
    ...
    dswrfAvg: Optional[float] = Field(
        None, description="Downward shortwave radiation avg (W/m²)"
    )
    dswrfMax: Optional[float] = Field(
        None, description="Downward shortwave radiation max (W/m²)"
    )
    dlwrfAvg: Optional[float] = Field(
        None, description="Downward longwave radiation avg (W/m²)"
    )
    dlwrfMax: Optional[float] = Field(
        None, description="Downward longwave radiation max (W/m²)"
    )
...
class AirQuality(BaseModel):
    ...
    aqiCategory: Optional[str] = Field(
    ...
    pollenTree: Optional[int] = Field(
        None, ge=0, le=4, description="Tree pollen level (0-4)"
    )
    pollenGrass: Optional[int] = Field(
        None, ge=0, le=4, description="Grass pollen level (0-4)"
    )
    pollenWeed: Optional[int] = Field(
        None, ge=0, le=4, description="Weed pollen level (0-4)"
    )
    moldLevel: Optional[int] = Field(None, ge=0, le=4, description="Mold level (0-4)")
    predominantPollen: Optional[str] = Field(
        None, description="Predominant pollen type"
    )
...
class SoilConditions(BaseModel):
    ...
    temp_0_10cm: Optional[float] = Field(
        None, description="Soil temperature 0-10cm depth (C)"
    )
    temp_10_40cm: Optional[float] = Field(
        None, description="Soil temperature 10-40cm depth (C)"
    )
    temp_40_100cm: Optional[float] = Field(
        None, description="Soil temperature 40-100cm depth (C)"
    )
    temp_100_200cm: Optional[float] = Field(
        None, description="Soil temperature 100-200cm depth (C)"
    )

    # Soil moisture (kg/m² or mm) at different depths
    moisture_0_10cm: Optional[float] = Field(
        None, description="Soil moisture 0-10cm depth (kg/m²)"
    )
    moisture_10_40cm: Optional[float] = Field(
        None, description="Soil moisture 10-40cm depth (kg/m²)"
    )
    moisture_40_100cm: Optional[float] = Field(
        None, description="Soil moisture 40-100cm depth (kg/m²)"
    )
    moisture_100_200cm: Optional[float] = Field(
        None, description="Soil moisture 100-200cm depth (kg/m²)"
    )
...
class CurrentWeather(BaseModel):
    ...
    precipitationProbability: Optional[int] = Field(
    ...
    windDirectionText: Optional[str] = Field(
        None, description="Wind direction (cardinal)"
    )
    ...
    airQualityIndex: Optional[int] = Field(
        None, description="Air quality index (EPA 0-500)"
    )
...
class AgriWeatherForecastDay(BaseModel):
    ...
    evapotranspiration: Optional[float] = Field(
        None, description="Reference evapotranspiration ET0 (mm)"
    )
    ...
    # Solar radiation
    solarRadiationAvg: Optional[float] = Field(
        None, description="Average solar radiation (W/m²)"
    )
    solarRadiationMax: Optional[float] = Field(
        None, description="Max solar radiation (W/m²)"
    )
...
class AgriWeatherForecast(BaseModel):
    ...
    days: List[AgriWeatherForecastDay] = Field(
...
class AgriculturalInsights(BaseModel):
    ...
    growingConditions: str = Field(
        ..., description="Overall growing conditions: excellent, good, fair, poor"
    )

    # Risk assessments
    frostRisk: str = Field(
        "none", description="Frost risk level: none, low, medium, high"
    )
    droughtRisk: str = Field(
        "none", description="Drought risk level: none, low, medium, high"
    )
    floodRisk: str = Field(
        "none", description="Flood risk level: none, low, medium, high"
    )
    heatStressRisk: str = Field(
        "none", description="Heat stress risk level: none, low, medium, high"
    )

    # Soil assessments
    soilWorkability: str = Field(
        "unknown", description="Soil workability: excellent, good, fair, poor, unknown"
    )
    irrigationNeed: str = Field(
        "unknown", description="Irrigation need: none, low, moderate, high, unknown"
    )

    # Recommendations
    recommendations: List[str] = Field(
        default_factory=list, description="List of agricultural recommendations"
    )

    # Alerts
    alerts: List[str] = Field(
        default_factory=list, description="Weather-related alerts for agriculture"
    )
...
class AgriWeatherData(BaseModel):
    ...
    # Forecast
    forecast: Optional[AgriWeatherForecast] = Field(
        None, description="Agricultural weather forecast"
    )

    # Insights
    insights: Optional[AgriculturalInsights] = Field(
        None, description="Agricultural insights and recommendations"
    )
