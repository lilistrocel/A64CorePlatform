...
async def get_dashboard_stats(
    ...
    employees_list, total_employees, _ = await employee_service.get_all_employees(
        page=1, per_page=100
    )

    # Get counts by status directly from the database for accuracy
    active_list, active_employees, _ = await employee_service.get_all_employees(
        page=1, per_page=1, status=EmployeeStatus.ACTIVE
    )
    on_leave_list, on_leave_employees, _ = await employee_service.get_all_employees(
        page=1, per_page=1, status=EmployeeStatus.ON_LEAVE
    )
    terminated_list, terminated_employees, _ = await employee_service.get_all_employees(
        page=1, per_page=1, status=EmployeeStatus.TERMINATED
    )
    ...
    recent_hires = [
        ...
        if hasattr(e, "hireDate")
        and e.hireDate
        and datetime.fromisoformat(
            str(e.hireDate).replace("Z", "+00:00").replace("+00:00", "")
        )
        > thirty_days_ago
    ][
        :5
    ...
    try:
        ...
        for employee in employees_list[:50]:  # Limit to avoid too many API calls
            try:
                ...
                visas, _, _ = await visa_service.get_employee_visas(
                    str(employee.employeeId), page=1, per_page=10
                )
                for visa in visas:
