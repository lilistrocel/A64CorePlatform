...
class SpacingDensity(BaseModel):
    ...
    plantsPerHundredSqm: int = Field(
        ..., gt=0, description="Number of plants per 100 m²"
    )
    description: Optional[str] = Field(
        None, description="Optional description for this spacing"
    )
...
class SpacingStandardsConfig(BaseModel):
    ...
    configType: str = Field(
        "spacing_standards", description="Configuration type identifier"
    )
...
def calculate_plant_count(
    ...
    plants_per_100sqm = densities.get(
...
def get_density_for_category(
    spacing_category: SpacingCategory, densities: Optional[Dict[str, int]] = None
) -> int:
    """
...
def area_m2_from_plants(plant_count: int, plants_per_100m2: int) -> float:
    ...
    if plants_per_100m2 <= 0:
        raise ValueError(f"plants_per_100m2 must be > 0, got {plants_per_100m2}")
...
def suggest_spacing_category(
    plant_name: str, plant_type: Optional[str] = None
) -> SpacingCategory:
    """
