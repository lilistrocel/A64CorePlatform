...
from ...utils.dashboard_calculator import (
...
async def get_dashboard_summary(
    ...
    farmIds: Optional[str] = Query(
        None, description="Comma-separated farm IDs to filter by"
    ),
    ...
    cropName: Optional[str] = Query(
        None, description="Filter by crop name (partial match, case-insensitive)"
    ),
    dateFrom: Optional[str] = Query(
        None, description="Filter harvests from this date (ISO format YYYY-MM-DD)"
    ),
    dateTo: Optional[str] = Query(
        None, description="Filter harvests up to this date (ISO format YYYY-MM-DD)"
    ),
    ...
    try:
        ...
        if dateFrom:
            try:
                ...
                raise HTTPException(
                    400, f"Invalid dateFrom format '{dateFrom}'. Expected YYYY-MM-DD."
                )
        if dateTo:
            try:
                ...
                raise HTTPException(
                    400, f"Invalid dateTo format '{dateTo}'. Expected YYYY-MM-DD."
                )
        ...
        blocks_by_state_result = await db.blocks.aggregate(
            blocks_by_state_pipeline
        ).to_list(None)

        # Convert to dict
        blocks_by_state_dict = {
            item["_id"]: item["count"] for item in blocks_by_state_result
        }
        ...
        blocks_by_farm_result = await db.blocks.aggregate(
            blocks_by_farm_pipeline
        ).to_list(None)
        ...
        for farm_data in blocks_by_farm_result:
            ...
            # Find farm name
            farm_name = next(
                (f["name"] for f in farms if f["farmId"] == farm_id), "Unknown Farm"
            )
        ...
        harvest_match_criteria = {"farmId": {"$in": farm_ids}}
        # Add farming year filter if specified
        if farmingYear is not None:
        ...
        harvests_by_farm_result = await db.block_harvests.aggregate(
            harvests_by_farm_pipeline
        ).to_list(None)
        ...
        predicted_pipeline = [
            {"$match": block_match_criteria},
            {
                "$group": {
                    "_id": "$farmId",
                    "predictedYieldKg": {
                        "$sum": {"$ifNull": ["$kpi.predictedYieldKg", 0]}
                    },
                }
            },
        ]
        ...
        archive_predicted_result = await db.block_archives.aggregate(
            archive_predicted_pipeline
        ).to_list(None)
        for item in archive_predicted_result:
            predicted_by_farm[item["_id"]] = (
                predicted_by_farm.get(item["_id"], 0) + item["predictedYieldKg"]
            )

        # Merge into per-farm KPI
        all_farm_ids_for_yield = set(
            list(actual_by_farm.keys()) + list(predicted_by_farm.keys())
        )
        yield_by_farm = []
        ...
        crop_actual_result = await db.block_harvests.aggregate(
            crop_actual_pipeline
        ).to_list(None)
        ...
        crop_predicted_result = await db.blocks.aggregate(
            crop_predicted_pipeline
        ).to_list(None)
        ...
        archive_crop_result = await db.block_archives.aggregate(
            archive_crop_pipeline
        ).to_list(None)
        ...
        farming_year_context = FarmingYearContext(
            farmingYear=farmingYear, isFiltered=farmingYear is not None
        )
        ...
        summary_data = DashboardSummaryData(
            ...
            harvestSummary=DashboardHarvestSummary(
                totalHarvestsKg=total_harvests_kg, harvestsByFarm=harvests_by_farm
            ),
        ...
        return DashboardSummaryResponse(success=True, data=summary_data)
...
async def get_filter_crops(
    farmingYear: Optional[int] = Query(
        None, description="Restrict crop names to blocks planted in this farming year"
    ),
    current_user: CurrentUser = Depends(get_current_user),
):
    """
...
async def get_farm_dashboard(
    farmId: UUID,
    farmingYear: Optional[int] = Query(
        None, description="Filter blocks by farming year planted"
    ),
    current_user: CurrentUser = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        active_physical_blocks = [
            ...
            if pb.state
            not in [BlockStatus.EMPTY, BlockStatus.CLEANING, BlockStatus.PARTIAL]
        ]
        ...
        logger.info(
            f"[Dashboard] Found {total} blocks for farm {farmId} ({virtual_total} virtual, {len(active_physical_blocks)} active physical of {len(physical_blocks)} total, farmingYear={farmingYear})"
        )
        ...
        for block in blocks:
            ...
            alerts_list, _ = await alert_repo.get_by_block(
                block.blockId, skip=0, limit=10
            )
        ...
        recent_activity = await get_recent_activity(
            farmId, days=7, farming_year=farmingYear
        )
        ...
        logger.error(
            f"[Dashboard] Error loading dashboard for farm {farmId}: {str(e)}",
            exc_info=True,
        )
...
async def quick_transition(
    ...
    try:
        ...
        updated_block = await BlockService.change_status(
            blockId, status_update, UUID(current_user.userId), current_user.email
        )
...
async def quick_harvest(
    ...
    try:
        ...
        if block.state != BlockStatus.HARVESTING:
            raise HTTPException(
                400, f"Block must be in HARVESTING state (current: {block.state.value})"
            )
        ...
        harvest_data = BlockHarvestCreate(
            ...
            farmId=farmId,
            plantingId=(
                block.currentPlantingId if hasattr(block, "currentPlantingId") else None
            ),
...
async def recalculate_kpi(
    farmId: Optional[UUID] = Query(
        None, description="Limit to a specific farm (omit for all farms)"
    ),
    current_user: CurrentUser = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        blocks = await db.blocks.find(
            block_filter, {"blockId": 1, "kpi": 1, "blockCode": 1}
        ).to_list(length=5000)
        ...
        for block_doc in blocks:
            ...
            try:
                ...
                agg_result = await db.block_harvests.aggregate(pipeline).to_list(
                    length=1
                )
                ...
                efficiency = (
                    round((actual_yield / predicted) * 100, 2) if predicted > 0 else 0.0
                )
...
async def get_recent_activity(
    farm_id: UUID, days: int = 7, farming_year: Optional[int] = None
                ...
                if change.changedAt >= cutoff_date:
                    activities.append(
                        DashboardActivity(
                            ...
                            or f"B{str(block.blockId)[:6].upper()}",
                            action="state_change",
