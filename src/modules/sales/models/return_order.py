...
class ReturnItem(BaseModel):
    ...
    orderItemId: UUID = Field(
        default_factory=uuid4, description="Unique ID for this return item"
    )
    originalOrderItemProductId: UUID = Field(
        ..., description="Product ID from original order item"
    )
    ...
    newGrade: Optional[str] = Field(
        None, description="New quality grade after return (may be downgraded)"
    )
    ...
    inventoryId: Optional[UUID] = Field(
        None, description="Original inventory ID (if linked)"
    )
    returnToInventory: bool = Field(
        True, description="Whether to return to inventory (False = waste)"
    )

    # Notes
    notes: Optional[str] = Field(
        None, max_length=500, description="Additional notes about this item"
    )
...
class ReturnOrderBase(BaseModel):
    ...
    shipmentId: Optional[UUID] = Field(
        None, description="Shipment ID (if return from delivery)"
    )
    ...
    returnDate: datetime = Field(
        default_factory=datetime.utcnow, description="Date return was initiated"
    )
    processedDate: Optional[datetime] = Field(
        None, description="Date return was processed"
    )

    # Totals
    totalReturnedQuantity: float = Field(0, ge=0, description="Total quantity returned")
    totalRefundAmount: Optional[float] = Field(
        None, ge=0, description="Total refund amount if applicable"
    )
...
class ReturnOrder(ReturnOrderBase):
    ...
    returnId: UUID = Field(
        default_factory=uuid4, description="Unique return order identifier"
    )
    returnCode: Optional[str] = Field(
        None, description="Human-readable return code (e.g., RET001)"
    )

    # Reference to original order details (denormalized for convenience)
    customerName: Optional[str] = Field(
        None, description="Customer name from original order"
    )
    orderCode: Optional[str] = Field(None, description="Original order code")

    # Processing
    processedBy: Optional[UUID] = Field(
        None, description="User ID who processed this return"
    )
...
class ProcessReturnResponse(BaseModel):
    ...
    inventoryUpdates: List[dict] = Field(
        default_factory=list, description="Inventory items updated"
    )
    wasteCreated: List[dict] = Field(
        default_factory=list, description="Waste records created"
    )
