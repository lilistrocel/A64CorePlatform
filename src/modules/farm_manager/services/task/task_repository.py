...
async def _enrich_tasks_with_block_farm(
    tasks: List[FarmTask],
    ...
    for task in tasks:
        ...
        expected_yield = (
            kpi.get("predictedYieldKg")
            if "predictedYieldKg" in kpi
            else (block or {}).get("expectedYieldKg")
        )
...
class TaskRepository:
    ...
    async def create(
        ...
        generated_from_cycle_id: Optional[UUID] = None,
    ) -> FarmTask:
        """
        ...
        if task_dict.get("taskData"):
            ...
            if task_data_obj.get("totalHarvest") and task_data_obj["totalHarvest"].get(
                "contributors"
            ):
        ...
        logger.info(
            f"Created farm task: {task.taskId} ({task.taskType}) for block {task.blockId}"
        )
        return task
    ...
    async def get_my_tasks(
        farm_id: UUID, user_id: UUID, status: Optional[TaskStatus] = None
    ) -> List[FarmTask]:
        """
    ...
    async def update(task_id: UUID, update_data: FarmTaskUpdate) -> Optional[FarmTask]:
        ...
        if "priority" in update_dict and isinstance(
            update_dict["priority"], TaskPriority
        ):
        ...
        result = await db.farm_tasks.update_one(
            {"taskId": str(task_id)}, {"$set": update_dict}
        )
    ...
    async def complete_task(
        task_id: UUID, user_id: UUID, user_email: str, task_data: TaskData
    ) -> Optional[FarmTask]:
        """
    ...
    async def add_harvest_entry(
        ...
        entry = HarvestEntry(
            **entry_data.model_dump(), userId=user_id, userEmail=user_email
        )
        ...
        logger.info(
            f"Added harvest entry to task {task_id}: {entry_data.quantity}kg grade {entry_data.grade}"
        )
        return await TaskRepository.get_by_id(task_id)
    ...
    async def aggregate_daily_harvest(task_id: UUID) -> Optional[FarmTask]:
        ...
        if task.taskData.harvestEntries:
            for entry in task.taskData.harvestEntries:
                ...
                grade_breakdown[grade_key] = (
                    grade_breakdown.get(grade_key, 0.0) + entry.quantity
                )
        ...
        harvest_total = HarvestTotal(
            ...
            entryCount=(
                len(task.taskData.harvestEntries) if task.taskData.harvestEntries else 0
            ),
        )
        ...
        if total_quantity > 0:
            ...
            try:
                ...
                harvester_email = (
                    task.taskData.harvestEntries[0].userEmail
                    if task.taskData.harvestEntries
                    else "system@a64core.com"
                )

                await HarvestService.record_harvest(
                    harvest_create, harvester_id, harvester_email
                )

                logger.info(
                    f"Created harvest record for task {task_id}: {total_quantity}kg (grade {best_grade.value})"
                )
            except Exception as e:
                logger.error(
                    f"Failed to create harvest record for task {task_id}: {e}",
                    exc_info=True,
                )
                # Don't fail the aggregation if harvest record creation fails

        logger.info(
            f"Aggregated daily harvest task {task_id}: {total_quantity}kg from {len(contributors)} farmers"
        )
        return await TaskRepository.get_by_id(task_id)

    @staticmethod
    async def get_daily_harvest_tasks_to_aggregate(
    ...
    @staticmethod
    async def cancel_future_tasks_for_cycle(
        cycle_id: UUID, task_types: List[TaskType]
    ) -> int:
        """
        ...
        logger.info(
            f"Cancelled {result.modified_count} future tasks for cycle {cycle_id}"
        )
        return result.modified_count

    @staticmethod
    async def reschedule_tasks_for_cycle(
        cycle_id: UUID, task_type: TaskType, new_scheduled_dates: List[datetime]
    ) -> int:
        """
        ...
        logger.info(
            f"Rescheduled {updated_count} {task_type} tasks for cycle {cycle_id}"
        )
