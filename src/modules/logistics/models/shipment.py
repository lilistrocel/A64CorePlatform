...
class CargoItem(BaseModel):
    ...
    description: str = Field(
        ..., min_length=1, max_length=500, description="Cargo description"
    )
...
class ShipmentBase(BaseModel):
    ...
    status: ShipmentStatus = Field(
        ShipmentStatus.PENDING, description="Shipment status"
    )
    scheduledDate: datetime = Field(..., description="Scheduled departure date")
    actualDepartureDate: Optional[datetime] = Field(
        None, description="Actual departure date"
    )
    actualArrivalDate: Optional[datetime] = Field(
        None, description="Actual arrival date"
    )
    cargo: List[CargoItem] = Field(
        default_factory=list,
        description="List of cargo items (can be auto-populated from orders)",
    )

    # Order linkage
    orderIds: List[UUID] = Field(
        default_factory=list, description="Sales Order IDs assigned to this shipment"
    )
    ...
    farmingYear: Optional[int] = Field(
        None,
        description="Farming year this shipment belongs to (calculated from departureTime or scheduledDate)",
    )
...
class ShipmentUpdate(BaseModel):
    ...
    farmingYear: Optional[int] = Field(
        None, description="Farming year (usually auto-calculated)"
    )
...
class Shipment(ShipmentBase):
    ...
    shipmentId: UUID = Field(
        default_factory=uuid4, description="Unique shipment identifier"
    )
    shipmentCode: Optional[str] = Field(
        None, description="Human-readable shipment code (e.g., SH001)"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "cargo": [
                    {"description": "Fresh produce", "quantity": 100, "weight": 25.5}
                ],
                "orderIds": [
...
class ShipmentTrackingData(BaseModel):
    ...
    destination: Optional[TrackingLocation] = Field(
        None, description="Destination GPS location"
    )
    currentLocation: Optional[TrackingLocation] = Field(
        None, description="Current estimated GPS location"
    )
    ...
    lastUpdated: datetime = Field(
        default_factory=datetime.utcnow, description="Last tracking update time"
    )
...
class OrderAssignmentResponse(BaseModel):
    ...
    totalCargoWeight: float = Field(
        ..., description="Total weight of cargo from orders"
    )
