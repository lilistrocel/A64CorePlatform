...
class RevenueBreakdown(BaseModel):
    ...
    unpaidAmount: float = Field(
        0.0, description="Total unpaid/outstanding amount (AED)"
    )
    collectionRate: float = Field(0.0, description="Paid / net revenue ratio (0–1)")
    bySource: Dict[str, float] = Field(
        default_factory=dict, description="Revenue broken down by metadata.priceSource"
    )
...
class CogsBreakdown(BaseModel):
    ...
    allocatedByCrop: float = Field(
        0.0, description="Purchase register items with mappedCropName (AED)"
    )
    allocatedByFarm: float = Field(
        0.0, description="Farm overhead allocated by revenue ratio (AED)"
    )
    unallocatedOverhead: float = Field(
        0.0, description="Overhead that could not be allocated (AED)"
    )
...
class OpexBreakdown(BaseModel):
    ...
    labor: float = Field(
        0.0, description="Labor costs — 0 until HR salary data is available"
    )
...
class KgSummary(BaseModel):
    ...
    harvested: float = Field(
        0.0, description="Total kg harvested (from block_harvests)"
    )
...
class PnLSummaryResponse(BaseModel):
    ...
    grossMarginPercent: float = Field(
        0.0, description="grossProfit / revenue.net * 100"
    )
    opex: OpexBreakdown
    operatingProfit: float = Field(0.0, description="grossProfit - opex.total (AED)")
    operatingMarginPercent: float = Field(
        0.0, description="operatingProfit / revenue.net * 100"
    )
    kg: KgSummary
    orders: OrderSummary
    period: PeriodInfo
...
class ARAgingResponse(BaseModel):
    ...
    aging_30_60: AgingBucket = Field(
        default_factory=AgingBucket, description="31–60 days"
    )
    aging_60_90: AgingBucket = Field(
        default_factory=AgingBucket, description="61–90 days"
    )
    over_90: AgingBucket = Field(
        default_factory=AgingBucket, description="Over 90 days"
    )
    total_outstanding: float = Field(
        0.0, description="Sum of all outstanding amounts (AED)"
    )
    byCustomer: List[CustomerAging] = Field(
        default_factory=list, description="Top 10 customers by outstanding"
    )
...
class RevenueSourcesResponse(BaseModel):
    ...
    imputed_customer_crop_avg: RevenueSourceEntry = Field(
        default_factory=RevenueSourceEntry
    )
