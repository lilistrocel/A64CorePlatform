...
class VehicleBase(BaseModel):
    ...
    licensePlate: str = Field(
        ..., min_length=1, max_length=50, description="License plate number"
    )
    ...
    rentalCostPerDay: Optional[float] = Field(
        None, ge=0, description="Rental cost per day"
    )
    ...
    maintenanceSchedule: Optional[date] = Field(
...
class Vehicle(VehicleBase):
    ...
    vehicleId: UUID = Field(
        default_factory=uuid4, description="Unique vehicle identifier"
    )
    vehicleCode: Optional[str] = Field(
        None, description="Human-readable vehicle code (e.g., V001)"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "capacity": {"weight": 5000, "volume": 30, "unit": "kg/m3"},
                "status": "available",
                "costPerKm": 2.5,
                "rentalCostPerDay": 0,
