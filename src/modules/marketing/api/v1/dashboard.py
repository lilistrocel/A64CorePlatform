...
async def get_dashboard_stats(
    ...
    try:
        ...
        budget_summary = (
            budget_stats[0]
            if budget_stats
            else {"totalBudgetAmount": 0, "totalAllocated": 0, "totalSpent": 0}
        )
        ...
        draft_campaigns = await campaigns_collection.count_documents(
            {"status": "draft"}
        )
        completed_campaigns = await campaigns_collection.count_documents(
            {"status": "completed"}
        )
        ...
        campaign_stats = await campaigns_collection.aggregate(
            campaign_pipeline
        ).to_list(1)
        campaign_summary = (
            campaign_stats[0]
        ...
        ctr = (
            (
                campaign_summary["totalClicks"]
                / campaign_summary["totalImpressions"]
                * 100
            )
            if campaign_summary["totalImpressions"] > 0
            else 0
        )
        conversion_rate = (
            (
                campaign_summary["totalConversions"]
                / campaign_summary["totalClicks"]
                * 100
            )
            if campaign_summary["totalClicks"] > 0
            else 0
        )
        budget_utilization = (
            (budget_summary["totalSpent"] / budget_summary["totalBudgetAmount"] * 100)
            if budget_summary["totalBudgetAmount"] > 0
            else 0
        )
        ...
        dashboard_data = {
            "budgets": {
                ...
                "remaining": budget_summary["totalBudgetAmount"]
                - budget_summary["totalSpent"],
        ...
        return SuccessResponse(
            data=dashboard_data, message="Dashboard statistics retrieved successfully"
        )
