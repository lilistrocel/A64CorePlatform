...
async def create_plant_data(
    ...
    **Validations**:
    ...
    plant = await PlantDataService.create_plant_data(
        plant_data, current_user.userId, current_user.email
    )

    return SuccessResponse(data=plant, message="Plant data created successfully")


@router.get("", response_model=PaginatedResponse[PlantData], summary="List plant data")
@cache_response(ttl=300, key_prefix="farm")
async def list_plant_data(
    page: int = Query(1, ge=1, description="Page number"),
    perPage: int = Query(20, ge=1, le=100, description="Items per page"),
    search: Optional[str] = Query(
        None, description="Search by plant name or scientific name"
    ),
    ...
    **Query Parameters**:
    ...
    plants, total = await PlantDataService.get_all_plant_data(
        page=page, per_page=perPage, search=search, is_active=isActive
    )
...
async def update_plant_data(
    ...
    **Validations**:
    ...
    updated_plant = await PlantDataService.update_plant_data(
        plant_data_id, update_data, current_user.userId
    )

    return SuccessResponse(
        data=updated_plant, message="Plant data updated successfully"
    )
...
async def import_plant_data_csv(
    ...
    updateExisting: bool = Query(
        False, description="Update existing plants or skip duplicates"
    ),
    current_user: CurrentUser = Depends(require_permission("agronomist")),
):
    """
    ...
    **Example CSV**:
    ...
    result = await PlantDataService.import_from_csv(
        file, current_user.userId, current_user.email, update_existing=updateExisting
    )
