...
router = APIRouter(
    prefix="/farms/{farm_id}/blocks/{block_id}/alerts", tags=["block-alerts"]
)
...
async def create_alert(
    ...
    changeBlockStatus: bool = Query(
        True, description="Whether to change block status to ALERT"
    ),
...
@router.get(
    "", response_model=PaginatedResponse[Alert], summary="List alerts for a block"
)
async def list_block_alerts(
    ...
    **Query Parameters**:
    ...
    alerts, total, total_pages = await AlertService.list_alerts_by_block(
        block_id, page=page, per_page=perPage, status=status_filter, severity=severity
    )
...
@router.get(
    "/{alert_id}", response_model=SuccessResponse[Alert], summary="Get alert by ID"
)
async def get_alert(
    farm_id: UUID,
    block_id: UUID,
...
async def resolve_alert(
    ...
    restoreBlockStatus: bool = Query(
        True, description="Whether to restore block to previousStatus"
    ),
    ...
    **Requirements**:
    ...
    return SuccessResponse(data=resolved_alert, message="Alert resolved successfully")
...
async def dismiss_alert(
    ...
    **Requirements**:
    ...
    dismissed_alert = await AlertService.dismiss_alert(
        alert_id, current_user.userId, current_user.email
    )

    return SuccessResponse(data=dismissed_alert, message="Alert dismissed successfully")


@router.delete(
    "/{alert_id}", response_model=SuccessResponse[dict], summary="Delete an alert"
)
async def delete_alert(
    ...
    **Use with caution**: This is a permanent deletion. Consider dismissing instead.
    ...
    return SuccessResponse(
        data={"alertId": str(alert_id)}, message="Alert deleted successfully"
    )
...
@farm_router.get(
    "", response_model=PaginatedResponse[Alert], summary="List all alerts in a farm"
)
async def list_farm_alerts(
    ...
    **Query Parameters**:
    ...
    alerts, total, total_pages = await AlertService.list_alerts_by_farm(
        farm_id, page=page, per_page=perPage, status=status_filter, severity=severity
    )
